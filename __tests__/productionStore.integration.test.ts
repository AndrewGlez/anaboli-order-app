...
import { isHistoricalVersion, getReadOnlyMessage } from "@/components/production/versionHistory";
import { isLegacyOrder, filterLegacyOrdersForDate, makeEligibleForReconciliation } from "@/components/production/legacyFixes";
...
describe("productionStore integration with UI helpers", () => {
  ...
  describe("UI/Store workflow", () => {
    it("full workflow: save report, view history, load historical, block save", () => {
      ...
      // Save initial report
      const result1 = useProductionStore.getState().saveReport("2024-01-15", quantities);
      ...
      // Save second version
      const result2 = useProductionStore.getState().saveReport("2024-01-15", quantities);
      ...
      const readOnlyMessage = getReadOnlyMessage(useProductionStore.getState().isReadOnly);
      expect(readOnlyMessage).toContain("sólo lectura");

      // Try to save (should be blocked)
      const result3 = useProductionStore.getState().saveReport("2024-01-15", quantities);
    ...
    it("legacy fix workflow: identify, filter, fix, verify", () => {
      ...
      const todaysLegacy = filterLegacyOrdersForDate(legacyOrders, "2024-01-15");
      expect(todaysLegacy).toHaveLength(1);

      // Step 3: Fix the legacy order
      const fixedOrder = makeEligibleForReconciliation(legacyOrders[0] as any, "Apple Pie");
