...
describe("tabConfig", () => {
	...
	it("has correct tab names", () => {
		...
		expect(TAB_ITEMS[2].name).toBe("gyms");
		expect(TAB_ITEMS[3].name).toBe("inventory");
		expect(TAB_ITEMS[4].name).toBe("settings");
	});

	it("has correct tab titles", () => {
		expect(TAB_ITEMS[0].title).toBe("Órdenes");
		expect(TAB_ITEMS[1].title).toBe("Producción");
		expect(TAB_ITEMS[2].title).toBe("Gimnasios");
		expect(TAB_ITEMS[3].title).toBe("Inventario");
		expect(TAB_ITEMS[4].title).toBe("Ajustes");
	});
});
